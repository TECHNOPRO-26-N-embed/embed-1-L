...
|:---|:---|:---|:---|:---|:---|
| 1 | readButton() | タクトスイッチを1回押す | true が返る | tureが変える | [ 合 ] |
| 2 | readButton() | スイッチを素早く2回押す | 1回分だけ true になる | 1回分だけtureになる | [ 合 ] |
| 3 |	updateLyrics() | lyricsIndex=配列長 |	エラー状態へ遷移する | エラー表示が出力 |	[ 合 ] |
| 4 |	playMelody() | melodyIndex=配列長 |	音が鳴らずエラー状態 | 音が鳴らずエラー状態 | [ 合 ] |
| 5 |	readButton() | 50ms未満で連打 | 1回しかtrueにならない | 1回しかtureにならない | [ 合 ] |
| 6 |	stop() | LCD未接続 | 音停止だけは必ず実行 | 音停止のみ実行 | [ 合 ] |

### 5-2. 出力系テスト

| No | テスト対象の関数 | 入力・操作 | 期待する結果 | 実際の結果 | 合否 |
|:---|:---|:---|:---|:---|:---|
| 1 | updateLyrics() | 歌詞切り替えタイミングで呼ぶ | LCDに次の歌詞が表示される| 次の歌詞が表示される | [ 合 ] |
| 2 | playMelody() | 曲再生中に呼ぶ | ブザーから正しい音が鳴る | ブザーから音が鳴る | [ 合 ] |
| 3 | stop() | 曲終了時に呼ぶ | LCDがクリアされ、音が止まる | LCDが止まり、音が止まる | [ 合 ] |
| 4 | loop() | currentState=0で待機 | LCdに初期メッセージが表示される | LCDに初期メッセージが表示される | [ 合 ] |
...
|:---|:---|:---|:---|:---|:---|
| 1 | 歌詞とメロディの平行動作 | 曲再生中にLCDとブザー両方を観察 | 歌詞と音がずれずに進む | 歌詞と音がずれずに進む | [ 合 ] |
